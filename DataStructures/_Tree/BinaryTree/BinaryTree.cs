...
namespace DataStructures._Tree.BinaryTree
{
	public class BinaryTree<T> where T:IComparable
	{
		...
		public Node<T> DeepestNode(Node<T> root)
		{
			...
			return temp;
		}//En derindeki düğümü geri döndüren fonksiyon
		...
		public static int NumberOfFullNodes(Node<T> root) => new BinaryTree<T>().LevelOrderNonRecursiveTraversal(root)
			...
			.ToList()
			.Count;//Tam düğümlerin sayısını veren fonksiyon
		public static int NumberOfFalfNodes(Node<T> root) => new BinaryTree<T>().LevelOrderNonRecursiveTraversal(root)
			.Where(node => (node.Left != null && node.Right == null) || (node.Left == null && node.Right != null))
			.ToList()
			.Count;//Yarım düğümlerin sayısını veren fonksiyon
		public void PrintPaths(Node<T> root)
		{
			var path = new T[256];
			PrintPaths(root, path, 0);//yaprak gördüğünde dizi içeriğini yazdırır
