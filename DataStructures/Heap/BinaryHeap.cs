...
namespace DataStructures.Heap
{
	public class BinaryHeap<T>: IEnumerable<T> where T : IComparable
	{
		...
		public T DeleteMinMax()
		{
			...
			var temp = Array[0];
			Array[0] = Array[position - 1];//position dizi üzerinde ekleme yapacağımız yeri gösterir
			position--;
			Count--;
			HeapifyDown();
			return temp;
		...
		protected void HeapifyUp()
		{
			...
			while (!IsRoot(index) && comparer.Compare(Array[index],GetParent(index))<0)//Array[index].CompareTo(GetParent(index)) < 0)
			{
