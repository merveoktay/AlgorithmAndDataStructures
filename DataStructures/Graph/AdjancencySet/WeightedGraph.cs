...
namespace DataStructures.Graph.AdjancencySet
{
	...
	{
		private Dictionary<T, WeightGraphVertex<T, TW>> vertices;
		...
		{
			vertices = new Dictionary<T, WeightGraphVertex<T, TW>>();
		}
		...
		public void AddVertex(T key)
		{
			...
			var newVartex = new WeightGraphVertex<T,TW>(key);
			vertices.Add(key, newVartex);
		}
		...
		public bool HasEdge(T source, T dest)
		{
			...
			if (!vertices.ContainsKey(source) || !vertices.ContainsKey(dest)) throw new ArgumentException("Source or destination is not in this graph.");
		...
		public void RemoveVertex(T key)
		{
			...
			foreach (var vertex in vertices[key].Edges)//Her defasında bir düğüm verir
		...
		private class WeightGraphVertex<T, TW> : IGraphVertex<T>
	        where TW : IComparable
		{
			public Dictionary<WeightGraphVertex<T, TW>, TW> Edges;
			public T Key { get; set; }
			public WeightGraphVertex(T key)
			{
				Key = key;
				Edges = new Dictionary<WeightGraphVertex<T, TW>, TW>();
			}
			IEnumerable<IEdge<T>> IGraphVertex<T>.Edges => Edges.Select(x => new Edge<T, TW>(x.Key, x.Value));
			public IEdge<T> GetEdge(IGraphVertex<T> targetVertex)
			{
				return new Edge<T, TW>(targetVertex,Edges[targetVertex as WeightGraphVertex<T,TW>]);
			}
			public IEnumerator<T> GetEnumerator()
			{
